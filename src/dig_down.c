...
static int dcr_corner_altitude(map_t *map, int i, int j)
{
...
static int dcr_square_alitude(interface_t *face, map_t *map, int i, int j)
{
...
void dig_down_square(interface_t *face, map_t *map)
{
    ...
    for (int i = map->rows - 2; i >= 0; i--) {
        for (int j = map->columns - 2; j >= 0 && n_return == 0; j--)
            n_return = dcr_square_alitude(face, map, i, j);
        if (n_return == 1)
            break;
    }
}
